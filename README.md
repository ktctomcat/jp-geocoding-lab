...
日本国内で無料で利用出来るジオコーディングAPIを叩くテスト用スクリプト。  
...
gc_single_script中にあるファイルは単体で動くテスト用のスクリプトです。ジオコードしたい住所録等は直接スクリプト内で記述します。    
gc_classはクラスで定義したものが保存されており、geocoding.pyからインポートして使う仕組みになっています。  
geocoding.pyの中に、ジオコードしたい住所録等を記述して実行させると、APIごとの結果の違いを比べることが出来ます。
