...
日本国内で利用できるジオコーディングAPIを叩くスクリプト各種

## 概要

gc_single_scriptと言うディレクトリの中に、それぞれ単体で動くテスト用のスクリプトか保存されています。  
ジオコードしたい住所録等は直接スクリプト内で記述する仕組みになっています。
gc_classはそれらをさらにクラスで定義したものが保存されており、geocode.pyでインポートして使う仕組みになっています。geocode.pyの中に、ジオコードしたい住所録等を記述して実行させると、APIごとの結果の違いを比べることが出来ます。
