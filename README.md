...
日本国内で利用出来るジオコーディングAPIを叩くテスト用スクリプト。

## 準備

requestsとxmltodictを使用していますので、あらかじめpip等でインストールしてください。

## 使い方

gc_single_scriptと言うディレクトリの中に、それぞれ単体で動くテスト用のスクリプトか保存されています。  
ジオコードしたい住所録等は直接スクリプト内で記述する仕組みになっていますので、直接編集して実行させてください。  
gc_classはそれらをさらにクラスで定義したものが保存されており、geocode.pyからインポートして使う仕組みになっています。  
geocode.pyの中に、ジオコードしたい住所録等を記述して実行させると、APIごとの結果の違いを比べることが出来ます。

## 採用しているジオコーディング API 一覧とリクエストURL及びレスポンスサンプル(参考)
